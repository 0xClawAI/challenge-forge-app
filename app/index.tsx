...
import { QUOTES } from '../src/data/templates';
...
function DailyScreen({ challengeId }: { challengeId: string }) {
  const { primary, accent, colors } = useTheme();
  const router = useRouter();
  const { challenges, dayResetHour, updateTaskLog, failChallenge, endChallenge, setActiveId } = useChallengeStore();
  ...
  const handleFail = useCallback(() => {
    Alert.alert(
      ...
      [
        ...
        { text: 'I Failed', style: 'destructive', onPress: () => failChallenge(ch.id) },
      ]
    );
  }, [ch, dn, failChallenge]);
  ...
  const handleEnd = useCallback(() => {
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        { text: 'End', onPress: () => endChallenge(ch.id) },
      ]
    );
  }, [ch, dn, endChallenge]);
